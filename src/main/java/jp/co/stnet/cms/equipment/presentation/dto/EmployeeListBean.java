...
public class EmployeeListBean extends EmployeeBean {
    ...
    public static EmployeeListBean of(EmployeeBean source) {
        var target = new EmployeeListBean();
        BeanUtils.copyProperties(source, target);

        target.setOperations(getOperations(source));
        target.setDT_RowId(source.getEmpId()); // DT_RowIDにはユニークな値をセットする
     ...
     * @return
     */
    static String getOperations(EmployeeBean source) {
        var op = new OperationsUtil(null);
        return "<a href=\"" + op.getEditUrl(source.getEmpId())
